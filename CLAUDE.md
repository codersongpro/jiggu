# CLAUDE.md

이 파일은 Claude Code가 이 프로젝트의 맥락을 이어받기 위한 문서다.
작업 시작 전에 이 문서와 `PRD.md`를 먼저 읽는다.

---

## 프로젝트 정체성

해외의류 **소싱 + 네이버 카페 핫딜 큐레이션** 자동화 도구.

운영자는 네이버 카페(해바라기)에 하루 6~10건씩 `[해외직구] 브랜드 상품명` 형식으로
핫딜 글을 올린다. 이 프로젝트의 목적은 두 가지다.

1. **갑자기 세일에 들어간 브랜드를 빠르게 찾는다** (시점 싸움)
2. **카페 게시글 제작 시간을 줄인다** (건당 15분 → 1분)

가격 수집기가 아니다. **소싱 판단 + 게시글 생산 도구**다.

---

## 확정된 결정

| 항목 | 결정 |
|---|---|
| 용도 | 재판매/셀러 소싱 기능(마진 계산·게시글 생성)을 그대로 유지하되, 실제 사용자는 운영자 본인과 가족 |
| 대상 지역 | 미국, 유럽, 일본 |
| 배포 | **Vercel** (정적 web/ + api/scan.js 서버리스 함수). 로컬 PC+스케줄러 전제(A3)는 폐기 |
| 스캔 방식 | 스마트폰에서 스캔 버튼 터치 → 온디맨드 실시간 조회. 자동 주기 스캔(크론)은 다음 단계 |
| 통관 모드 | `proxy`(구매대행)와 `business`(사업자 사입) 둘 다 지원. 기본값 proxy |
| 산출물 | 모바일 웹 + 게시글 패키지 (엑셀 출력은 미완성) |
| 발행 방식 | **자동 발행 불가** (네이버 글쓰기 API 종료). 복붙 + 이미지 공유가 최선 |
| 구입처 결정 | 이번 1차 버전은 사이트 하나당 하나의 소스만 본다. 여러 몰 교차 매칭·최저가 자동 선택은 다음 단계 |

---

## 절대 잊으면 안 되는 도메인 규칙

원가 계산이 틀리면 도구 전체가 무의미해진다. 아래는 실제로 자주 틀리는 지점이다.

1. **유럽 표시가는 VAT 포함가다.** 한국 배송 시 차감되므로 `표시가 ÷ (1+VAT)`로 실결제액을 구한다.
   이걸 빼먹으면 원가가 약 20% 틀어진다. 일본 세込 표기도 동일(÷1.1).
2. **면세한도 초과 시 초과분이 아니라 전체 금액에 과세**된다. 미국발 $200, 그 외 $150.
3. **사업자 사입에는 면세한도가 없다.** 같은 상품도 통관 모드에 따라 원가가 25% 차이난다.
4. 환율은 매매기준율이 아니라 **카드 수수료를 더한 실질환율**을 쓴다.
5. 관세·요율은 반드시 `config/cost.yaml`에서만 관리한다. **코드에 숫자를 넣지 않는다.**

---

## 아키텍처 (Vercel 배포 버전, 1차)

```
[수집]   스마트폰 스캔 버튼 → api/scan.js(Vercel 서버리스) → config/targets.json에 등록된
         Shopify 사이트들의 products.json을 병렬 조회 (온디맨드, 자동 주기 스캔 아님)
   ↓
[정규화] api/scan.js가 즉시 처리 — 사이즈/컬러/무게 추출, product_type으로 카테고리 보정
   ↓
[브랜드 판별] product.vendor 필드를 우선 사용 → 멀티브랜드 편집숍도 브랜드 사전 등록 없이 잡힌다
   ↓
[원가]   api/_lib/landedCost.js  ← core/landed_cost.py를 Node로 포팅, 1원 단위까지 결과 일치 검증됨
   ↓
[출력]   모바일 웹(web/index.html) — 원가 상세 / 구입처 / 이미지 선택 / 게시글 생성
```

여러 몰의 같은 상품 매칭(최저가 구입처 자동 확정), 어제 대비 세일 급변 감지(신호),
SQLite 저장은 이번 1차 범위 밖이다 — 아래 "미완성" 참고.

### 브랜드 공식몰 vs 멀티브랜드 편집숍
`config/targets.json`은 두 목록을 따로 둔다.
- `sites`: 브랜드 공식몰. `brand` 필드가 곧 그 사이트가 파는 브랜드
- `sites_multi_brand`: 여러 브랜드를 한 사이트에서 파는 편집숍(예: Bandier). `brand` 필드는
  구입처(사이트) 이름일 뿐이고, 실제 상품 브랜드는 Shopify 응답의 `vendor` 필드에서 자동으로
  읽는다 — **그래서 목록에 없는 브랜드가 갑자기 세일해도 미리 등록하지 않아도 잡힌다.**
  단, 안티봇이 있는 대형 종합몰(Nordstrom Rack, Saks OFF 5TH 등)은 이 방식으로도 못 잡는다
  (아래 리스크 참고).

### 이전 계획(폐기) — 참고용
아래는 로컬 PC + 크론 전제였던 PRD.md 원안이다. Vercel 온디맨드 스캔으로 전환하며 폐기했지만,
2단 스캔·세일 감지·상품 매칭 아이디어는 저장소를 붙이는 다음 단계에서 재사용할 수 있다.

- **라이트 스캔** 30분~1시간: 브랜드별 세일 카테고리 첫 페이지만. Shopify면 `/collections/sale/products.json?limit=250` 한 번
- **풀 스캔** 하루 1회 새벽: 전체 상품 + 사이즈별 재고

### 세일 감지 규칙
| 신호 | 조건 |
|---|---|
| 세일 개시 | 세일 상품 비율 +15%p 이상 |
| 추가 세일 | 평균 할인율 +8%p 이상 |
| 품목 확대 | 세일 상품 수 +30% 이상 |
| 개별 핫딜 | 단품 할인율 40% 이상 |

쿨다운: 브랜드 12시간, 상품 24시간.

### 상품 매칭 우선순위
1. 제조사 품번(SKU) 2. 바코드(EAN/UPC) 3. **이미지 지문(pHash)** 4. 브랜드+상품명 유사도

3번이 실전에서 잘 통한다. 편집숍들이 브랜드 룩북 이미지를 공유하기 때문이다.
4번으로 매칭된 건은 `confidence: name`으로 표시하고 사람이 확인하게 둔다.

---

## 브랜드 세일 성격 (스캔 설정에 반영할 것)

| 유형 | 브랜드 | 감지 전략 |
|---|---|---|
| 상시 아울렛 | 코치, 케이트스페이드, 마이클코어스, 토리버치, 폴로 | "세일 개시"가 안 뜬다. **추가 인하**를 본다 |
| 시즌 세일 | 산드로, 마쥬, 쟈딕앤볼테르, 띠어리, 가니, 올세인츠 | **"세일 개시" 감지가 핵심.** 1월·7월 집중 |
| 세일 거의 없음 | 아크테릭스, 파타고니아, 룰루레몬, 골든구스 | 스캔 주기를 낮추거나 제외. 룰루레몬은 상시 세일 코너만 |

---

## 현재 상태

### 완성
- `core/landed_cost.py` — 원가·마진 계산 엔진(Python, 참고/검증용). 실행 검증 완료
- `api/_lib/landedCost.js` — 위 로직의 Node 포팅. Python과 1원 단위까지 결과 일치 검증
- `config/cost.yaml` — 관세·환율·배대지·수수료 설정
- `config/targets.json` — 스캔 대상 (브랜드 공식몰 25곳 + 멀티브랜드 편집숍 4곳). `verified`
  필드로 Shopify 사용 확신도 표시. **이 세션은 외부망이 막혀 있어 실제 접속 검증을 못했다 —
  배포 후 스캔해서 `errors`에 뭐가 실패하는지 반드시 확인할 것**
- `api/scan.js` — Vercel 서버리스 스캔 함수. `products.json` 병렬 조회, `vendor` 필드로 멀티
  브랜드 자동 판별, `product_type`으로 카테고리 자동 보정, 사이트 하나 실패해도 나머지는 진행
- `web/index.html` — 모바일 프론트. `/api/scan` 연동 완료
  - 세일 신호(이번 스캔 요약) / 검색+자동추천 / 원가 상세 / 구입처 / 이미지 선택 / 게시글 생성
  - 원가 계산과 게시글 생성은 **실제로 동작**한다
- `vercel.json`, `package.json` — Vercel 배포 설정

### 미완성 — 여기서부터 작업
- [ ] 여러 몰 교차 매칭 → 최저가 구입처 자동 선택 (지금은 사이트 하나당 소스 하나만 본다)
- [ ] 스캔 결과 저장소(Vercel Postgres/KV 등) → 어제 대비 세일 급변 감지("신호") 복원
- [ ] `core/normalize.py` 격 — 브랜드·사이즈·스타일코드 표준화 (지금은 vendor 필드 그대로 씀)
- [ ] 국내 시세 매칭(네이버쇼핑) → 마진 랭킹
- [ ] `output/excel.py`
- [ ] 텔레그램 알림
- [ ] 안티봇 있는 대형 종합몰(Nordstrom Rack, Saks OFF 5TH 등) — 브라우저 자동화가 필요해
      Vercel 범위 밖. 로컬 도구(`byungjunjang/web-crawler` 등)로 별도 수집 후 업로드하는
      구조를 추가하면 확장 가능
- [ ] 상품명 한글 번역 (지금은 영문 원문 그대로 표시)

### 아직 결정 안 된 것 — 진행 전에 사용자에게 물어야 함
1. 카페 게시글 **본문 실제 형식** (현재 템플릿은 추정본이다)
2. 배대지 업체 → `cost.yaml` 요율표를 실제값으로 교체
3. `config/targets.json`의 `verified: unconfirmed` 사이트들 — 배포 후 스캔해서 실제로 걸리는
   곳만 남기고 정리할 것

---

## 작업 규칙

- 설명과 주석은 한국어로 쓴다.
- 요청된 것만 만든다. 추측성 기능을 넣지 않는다.
- 기존 코드는 요청된 부분만 수정한다. 전체를 갈아엎지 않는다.
- 가정이 필요하면 먼저 명시하고, 모호하면 질문한다.
- API 키·토큰은 코드에 넣지 않고 환경변수로 관리한다.
- 수집 요청은 저빈도·저동시성으로 한다 (도메인당 동시 1~2, 간격 2~5초 랜덤).
- 웹은 모바일 우선 반응형. PC·태블릿까지 자연스럽게 확장.
- 프론트엔드는 외부 이미지·스크립트 의존을 최소화한다.

---

## 리스크

- **ToS**: 재판매 목적 스크래핑은 개인 참고용보다 리스크가 크다. 공개 API와 제휴 상품 피드를 우선한다.
- **상품 이미지 저작권**: 브랜드·판매몰에 권리가 있다. 카페 규모가 커지면 자체 촬영컷 전환을 검토한다.
- **안티봇**: SSENSE·Farfetch·Nike 계열은 Akamai/PerimeterX. 초기 범위에서 제외한다.
- **관세 규정 변동**: `cost.yaml`의 `meta.last_verified`를 분기마다 갱신한다.
