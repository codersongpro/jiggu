...
| 탭 | 트리거 | 대상 | 갱신되는 것 |
|---|---|---|---|
| **Sale중** | 헤더 우측 "스캔" 버튼 | `config/targets.json`의 `popular: true` 사이트만(대중적 브랜드 위주) | 세일 신호(SIGNALS)만. 상품탭은 안 건드림 |
| **상품** | 상품탭 드롭다운 옆 "확인" 버튼 | 브랜드를 지정하면 그 사이트 **캡 없이 전체**, "브랜드 전체"면 등록된 모든 사이트(캡 적용) | 상품 목록(ITEMS)만. Sale중은 안 건드림 |

브랜드 드롭다운에서 특정 브랜드를 고르고 확인을 누르면 할인 여부와 무관하게 **그 브랜드 사이트의 상품을 전부** 가져온다 — 세일이 아니거나 저가 30% 안에 못 들어서 일반 스캔에서 빠졌던 상품(예: 특정 브랜드의 특정 종류)도 이렇게 하면 찾을 수 있다.
...
| `config/targets.json` | 스캔 대상 브랜드몰 (도메인·지역·통화·카테고리). Shopify `products.json`을 쓰는 사이트만 추가 가능. `popular: true`를 붙이면 Sale중 탭 스캔 대상에 포함된다 |
